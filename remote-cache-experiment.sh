...
echo "------------------------------------------------------------"
set -x
./gradlew $task -g $homeDir -Dscan.tag.remote-cache-experiment-init --no-configuration-cache -Ddevelocity.deprecation.muteWarnings=true -Dscan.uploadInBackground=false -Dgradle.enterprise.url=https://ge.solutions-team.gradle.com/ -Dorg.gradle.dependency.verification=lenient
set +x

runs='transforms kotlin-dsl transforms-selected'
for run in $runs
do
    ...
    then
        cache='transforms'
        disabledCacheArgs='-Dorg.gradle.internal.transform-caching-disabled=com.android.build.gradle.internal.dependency.ExtractJniTransform,com.android.build.gradle.internal.dependency.AarResourcesCompilerTransform,com.android.build.gradle.internal.dependency.LibrarySymbolTableTransform,com.android.build.gradle.internal.dependency.EnumerateClassesTransform,org.jetbrains.kotlin.gradle.internal.transforms.ClasspathEntrySnapshotTransform'
    fi
